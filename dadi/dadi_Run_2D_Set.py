...
#Parse arguments
parser = argparse.ArgumentParser(description='One argument')
parser.add_argument('-prefix', dest = 'prefix', action='store', nargs = 1, type = str, help = 'outputFile prefix')
...
fs = dadi.Spectrum.from_file("/home/vsoni11/aye_aye/pop0-pop1.sfs")
...
# Bottleneck, then split with no migration, followed by gradual size change.
Optimize_Functions.Optimize_Routine(fs, pts, prefix, "bottleneck_split_sizeChange", Models_2D.bottleneck_split_sizeChange, rounds, 6, fs_folded=fs_folded,
                                        reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1b, nu2b, Ts, Tg, nu1f, nu2f")
